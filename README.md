...
> [!TIP]
> - μ - это вектор поканальных средних по всем $h_{t}$: $μ = \frac{1}{T}\sum_{t=1}^{T} h_t$
...
2) Значения $e_{t, c}$ нормализуются функцией `softmax` независимо по каждому каналу по всем временным фрагментам $t \in [1, T]$ по формуле (2):
...
Кроме того вы можете использовать готовый скрипт `вставить сюда имя скрипта` из репозитория исследования для формирования собственного датасета из VoxTube (например, если вы хотите загрузить другой язык или увеличить количество аудиозаписей на диктора). 

Инструкция по запуску скрипта:
`здесь описание параметров запуска`
